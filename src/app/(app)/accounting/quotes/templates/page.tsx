...
export default function QuoteTemplatesPage() {
  ...
  const handleDeleteTemplate = (templateName: string) => {
    if (window.confirm(`Are you sure you want to delete the "${templateName}" template?`)) {
      ...
      localStorage.setItem(QUOTE_TEMPLATES_KEY, JSON.stringify(updatedTemplates));
      toast({
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="text-center mb-6">
        <Button asChild>
          <Link href="/accounting/quotes/create">
      ...
      {templates.length > 0 ? (
        ...
        <div className="text-center text-muted-foreground py-16 border-2 border-dashed rounded-lg">
          ...
          <p className="mt-4">
            No templates found. Create one from the quote generator to get started.
          </p>
        </div>
      )}
    </div>
