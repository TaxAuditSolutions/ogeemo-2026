...
const TrashDropZone = () => {
  ...
  const [{ isOver, canDrop }, drop] = useDrop(() => ({
    ...
    drop: async (item: ActionChipData & { index: number }) => {
      if (!user) return;
      try {
        await trashActionChips(user.uid, [item], 'hr');
        toast({
...
export default function ManageHrNavigationPage() {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('hrChipsUpdated', handleChipsUpdate);
    return () => window.removeEventListener('hrChipsUpdated', handleChipsUpdate);
  }, [loadChips]);
  ...
  const handleTrashChip = async (chipToTrash: ActionChipData) => {
    if (!user) return;
    try {
      await trashActionChips(user.uid, [chipToTrash], 'hr');
      toast({
        title: 'Action Trashed',
  ...
  const handleSaveUserChipOrder = async () => {
    if (!user) return;
    try {
      await updateActionChips(user.uid, chipsState.userChips, 'hr');
      await updateAvailableActionChips(user.uid, chipsState.availableChips, 'hr');
      window.dispatchEvent(new Event('hrChipsUpdated'));
      toast({
  ...
  return (
    <>
      <div className="p-4 sm:p-6 space-y-6">
        <HrPageHeader pageTitle="Manage HR Navigation" />
        <header className="text-center">
            <h1 className="text-2xl font-bold font-headline text-primary">Manage HR Quick Navigation</h1>
            <p className="text-muted-foreground max-w-2xl mx-auto">
                Drag and drop actions to customize the HR Hub's quick navigation menu.
            </p>
        </header>
