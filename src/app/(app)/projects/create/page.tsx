...
import { addProject, getProjectTemplates, updateProjectTemplate, deleteProjectTemplate, type Project, getProjectById, updateProject } from '@/services/project-service';
...
import { type Event as TaskEvent, type ProjectTemplate, type ProjectStep } from '@/types/calendar-types';

type ProjectTemplateWithLegacySteps = ProjectTemplate & { steps?: Partial<ProjectStep>[] };
...
function ProjectCreateForm() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const templatesData = await getProjectTemplates(user.uid).catch(() => []);
      ...
      toast({ variant: 'destructive', title: 'Error', description: 'Failed to load contacts for project creation.' });
    } finally {
      setIsLoadingData(false);
    }
  }, [user, toast]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (projectToEditId) {
        ...
        const templateSteps = (selectedTemplate as ProjectTemplateWithLegacySteps | undefined)?.steps || [];
