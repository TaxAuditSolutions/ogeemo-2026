...
const EDIT_QUOTE_ID_KEY = 'editQuoteId';
...
const QuoteDocument = ({
  ...
  <div className="p-12 bg-white text-black min-h-[11in] w-full max-w-[8.5in] mx-auto shadow-sm">
    ...
    <section className="flex justify-between mt-8">
      <div>
        <h2 className="font-bold text-gray-500 uppercase mb-2 text-xs tracking-widest">Quote For</h2>
      ...
      <div className="text-right space-y-1">
        <p className="text-sm"><span className="font-bold text-gray-500 uppercase text-xs mr-2">Quote Date:</span> {format(quoteDate, 'PP')}</p>
        <p className="text-sm"><span className="font-bold text-gray-500 uppercase text-xs mr-2">Valid Until:</span> {format(expirationDate, 'PP')}</p>
      </div>
    </section>
    ...
    <section className="flex justify-end mt-10">
      <div className="w-full max-w-sm space-y-3">
        ...
        <div className="flex justify-between font-bold text-xl py-2">
          <span className="uppercase text-xs self-center">Total Amount</span>
    ...
    <section className="mt-12 pt-8 border-t border-gray-300">
      <h4 className="font-bold text-gray-500 uppercase mb-6 text-xs tracking-widest">Client Authorization</h4>
      <div className="grid grid-cols-2 gap-x-12 gap-y-8 text-sm text-gray-700">
        <div>
          <span className="font-bold text-gray-500 uppercase text-xs">Authorizer's Name:</span>
          <div className="border-b border-gray-400 mt-6 w-full"></div>
        </div>
        <div>
          <span className="font-bold text-gray-500 uppercase text-xs">PO Number:</span>
          <div className="border-b border-gray-400 mt-6 w-full"></div>
        </div>
        <div>
          <span className="font-bold text-gray-500 uppercase text-xs">Approved (Signature):</span>
          <div className="border-b border-gray-400 mt-8 w-full"></div>
        </div>
        <div>
...
export function QuoteGeneratorView() {
  ...
  const loadQuoteForEditing = useCallback(async (quoteId: string) => {
    ...
    try {
      ...
      if (!quoteData) {
        toast({ variant: 'destructive', title: 'Error', description: 'Could not find the quote to edit.' });
        localStorage.removeItem(EDIT_QUOTE_ID_KEY);
        return;
      }

      setQuoteNumber(quoteData.quoteNumber);
      setBusinessNumber(quoteData.businessNumber || '');
  ...
  useEffect(() => {
    async function initializeView() {
      ...
      try {
        ...
        const quoteId = localStorage.getItem(EDIT_QUOTE_ID_KEY);
        if (quoteId) {
    ...
    initializeView();
  }, [user, toast, loadQuoteForEditing, searchParams]);
  ...
  const handleSaveQuote = async () => {
    ...
    try {
      ...
      if (quoteToEditId) {
        ...
        await addQuoteWithLineItems(quoteData, itemsToSave);
  ...
  return (
    <>
      <div className="p-4 sm:p-6 space-y-6 text-black bg-background min-h-screen">
        <InvoicePageHeader pageTitle="Create Quote" hubPath="/accounting/quotes" hubLabel="Quotes" />
        <header className="relative text-center print:hidden">
          <h1 className="text-3xl font-bold font-headline text-primary">Create a Quote</h1>
                ...
                <div className="space-y-2">
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0">
                      <CustomCalendar mode="single" selected={quoteDate} onSelect={(date) => date && setQuoteDate(date)} initialFocus />
                    </PopoverContent>
                  </Popover>
                </div>
                <div className="space-y-2">
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0">
                      <CustomCalendar mode="single" selected={expirationDate} onSelect={(date) => date && setExpirationDate(date)} initialFocus />
                    </PopoverContent>
                  </Popover>
                </div>
              </div>
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent side="top" align="start">
                          <p className="max-w-xs text-xs">
                            This note will appear on the quote and help your client understand next steps.
                          </p>
                        </TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
        ...
        <DialogContent className="max-w-5xl h-[90vh] flex flex-col p-0 overflow-hidden bg-muted/30">
          <DialogHeader className="p-4 border-b bg-background shrink-0 text-black">
            <div className="flex justify-between items-center">
              <DialogTitle>High-Fidelity Quote Preview</DialogTitle>
