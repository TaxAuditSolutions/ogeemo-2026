                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="flex-1 flex flex-col min-h-0">
                        <ScrollArea className="flex-1">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 px-6 py-6 bg-card">
                                ...
                                <section className="md:col-span-2 space-y-4">
                                    <h3 className="font-semibold text-lg border-b pb-1">Business & Tax Details</h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <FormField control={form.control} name="businessName" render={({ field }) => ( <FormItem> <FormLabel>Company / Business Name</FormLabel><Popover open={isCompanyPopoverOpen} onOpenChange={setIsCompanyPopoverOpen}><PopoverTrigger asChild><FormControl><Button variant="outline" role="combobox" className="w-full justify-between"><span className="truncate">{field.value || "Select a company..."}</span><ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50"/></Button></FormControl></PopoverTrigger><PopoverContent className="w-[--radix-popover-trigger-width] p-0"><Command filter={(value, search) => value.toLowerCase().includes(search.toLowerCase()) ? 1 : 0}><CommandInput placeholder="Search or create..." value={companySearchValue} onValueChange={setCompanySearchValue}/><CommandList><CommandEmpty><div className="p-1"><Button variant="outline" className="w-full" onClick={() => { handleCreateCompany(companySearchValue); setIsCompanyPopoverOpen(false); }}><Plus className="mr-2 h-4 w-4"/> Create "{companySearchValue}"</Button></div></CommandEmpty><CommandGroup>{companies?.map(c => ( <CommandItem key={c.id} value={c.name} onSelect={() => { form.setValue('businessName', c.name); setIsCompanyPopoverOpen(false); }}> <Check className={cn("mr-2 h-4 w-4", field.value === c.name ? 'opacity-100' : 'opacity-0')} /> {c.name}</CommandItem>))}</CommandGroup></CommandList></Command></PopoverContent></Popover><FormMessage /></FormItem>)} />
                                        <FormField control={form.control} name="craProgramAccountNumber" render={({ field }) => ( <FormItem> <FormLabel>CRA Program Account # (BN)</FormLabel> <FormControl><Input placeholder="e.g., 123456789RP0001" {...field} /></FormControl> <FormMessage /> </FormItem> )} />
                                    </div>
                                    <FormField control={form.control} name="industryCode" render={({ field }) => ( <FormItem className="flex flex-col"><FormLabel>Industry Classification (NAICS)</FormLabel><div className="flex gap-2"><Popover open={isIndustryPopoverOpen} onOpenChange={setIsIndustryPopoverOpen}><PopoverTrigger asChild><FormControl><Button variant="outline" role="combobox" className="w-full justify-between"><span className="truncate">{field.value ? allIndustries.find(i => i.code === field.value)?.description : 'Select an industry...'}</span><ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" /></Button></FormControl></PopoverTrigger><PopoverContent className="w-[--radix-popover-trigger-width] p-0"><Command filter={(value, search) => value.toLowerCase().includes(search.toLowerCase()) ? 1 : 0}><CommandInput placeholder="Search industry..." value={industrySearchValue} onValueChange={setIndustrySearchValue}/><CommandList><CommandEmpty>No industry found.</CommandEmpty><CommandGroup>{allIndustries.map(i => { const isCustom = 'userId' in i; return ( <CommandItem key={i.code} value={`${i.description} ${i.code}`} onSelect={() => { form.setValue('industryCode', i.code); setIsIndustryPopoverOpen(false); }} className="flex justify-between items-center group"> <div className="flex items-center"> <Check className={cn("mr-2 h-4 w-4", field.value === i.code ? 'opacity-100' : 'opacity-0')} /> <span>({i.code}) {i.description}</span></div> {isCustom && ( <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6 opacity-0 group-hover:opacity-100" onClick={(e) => e.stopPropagation()}><MoreVertical className="h-4 w-4"/></Button></DropdownMenuTrigger><DropdownMenuContent onClick={(e) => e.stopPropagation()}><DropdownMenuItem onSelect={() => handleOpenCreateIndustryDialog(i as Industry)}><Edit className="mr-2 h-4 w-4"/> Edit</DropdownMenuItem><DropdownMenuItem onSelect={() => setIndustryToDelete(i as Industry)} className="text-destructive"><Trash2 className="mr-2 h-4 w-4"/> Delete</DropdownMenuItem></DropdownMenuContent></DropdownMenu>)}</CommandItem>);})}</CommandGroup></CommandList></Command></PopoverContent></Popover><Button type="button" variant="outline" size="icon" onClick={() => handleOpenCreateIndustryDialog(null)}><Plus className="h-4 w-4" /></Button></div><FormMessage /></FormItem>)}/>
                                </section>

                                <Separator className="md:col-span-2 my-2" />

                                <section className="md:col-span-2 space-y-4">
                                    <h3 className="font-semibold text-lg border-b pb-1">Contact Information</h3>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                        <FormField control={form.control} name="businessPhone" render={({ field }) => ( <FormItem><FormLabel>Business #</FormLabel><div className="relative"><FormControl><Input placeholder="123-456-7890" {...field} /></FormControl>{field.value && <Button type="button" variant="ghost" size="icon" className="absolute right-1 top-1/2 h-8 w-8 -translate-y-1/2" asChild><a href={`tel:${field.value}`}><Phone className="h-4 w-4" /></a></Button>}</div></FormItem>)} />
                                        <FormField control={form.control} name="cellPhone" render={({ field }) => ( <FormItem><FormLabel>Cell #</FormLabel><div className="relative"><FormControl><Input placeholder="123-456-7890" {...field} /></FormControl>{field.value && <Button type="button" variant="ghost" size="icon" className="absolute right-1 top-1/2 h-8 w-8 -translate-y-1/2" asChild><a href={`tel:${field.value}`}><Phone className="h-4 w-4" /></a></Button>}</div></FormItem>)} />
                                        <FormField control={form.control} name="homePhone" render={({ field }) => ( <FormItem><FormLabel>Home #</FormLabel><div className="relative"><FormControl><Input placeholder="123-456-7890" {...field} /></FormControl>{field.value && <Button type="button" variant="ghost" size="icon" className="absolute right-1 top-1/2 h-8 w-8 -translate-y-1/2" asChild><a href={`tel:${field.value}`}><Phone className="h-4 w-4" /></a></Button>}</div></FormItem>)} />
                                    </div>
                                    <FormField control={form.control} name="primaryPhoneType" render={({ field }) => ( <FormItem className="space-y-2"><FormLabel>Preferred Contact Number</FormLabel><FormControl><RadioGroup onValueChange={field.onChange} value={field.value || ""} className="grid grid-cols-1 sm:grid-cols-3 gap-2"><FormItem className={cn("flex items-center space-x-3 space-y-0 rounded-md border p-2", field.value === 'businessPhone' ? "bg-primary/10 border-primary" : "")}><FormControl><RadioGroupItem value="businessPhone" disabled={!businessPhoneValue} /></FormControl><FormLabel className="font-normal w-full cursor-pointer">Business</FormLabel></FormItem><FormItem className={cn("flex items-center space-x-3 space-y-0 rounded-md border p-2", field.value === 'cellPhone' ? "bg-primary/10 border-primary" : "")}><FormControl><RadioGroupItem value="cellPhone" disabled={!cellPhoneValue} /></FormControl><FormLabel className="font-normal w-full cursor-pointer">Cell</FormLabel></FormItem><FormItem className={cn("flex items-center space-x-3 space-y-0 rounded-md border p-2", field.value === 'homePhone' ? "bg-primary/10 border-primary" : "")}><FormControl><RadioGroupItem value="homePhone" disabled={!homePhoneValue} /></FormControl><FormLabel className="font-normal w-full cursor-pointer">Home</FormLabel></FormItem></RadioGroup></FormControl></FormItem>)} />
                                </section>

                                <Separator className="md:col-span-2 my-2" />

                                <section className="md:col-span-2 space-y-4">
                                    <h3 className="font-semibold text-lg border-b pb-1">Addresses</h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-2 p-4 border rounded-md">
                                            <Label className="text-primary">Business Address</Label>
                                            <FormField control={form.control} name="streetAddress" render={({ field }) => ( <FormItem><FormLabel className="text-xs">Street</FormLabel><FormControl><Input placeholder="123 Main St" {...field} /></FormControl></FormItem> )} />
                                            <div className="grid grid-cols-2 gap-2">
                                                <FormField control={form.control} name="city" render={({ field }) => ( <FormItem><FormLabel className="text-xs">City</FormLabel><FormControl><Input {...field} /></FormControl></FormItem> )} />
                                                <FormField control={form.control} name="provinceState" render={({ field }) => ( <FormItem><FormLabel className="text-xs">Prov/State</FormLabel><FormControl><Input {...field} /></FormControl></FormItem> )} />
                                            </div>
                                            <div className="grid grid-cols-2 gap-2">
                                                <FormField control={form.control} name="postalCode" render={({ field }) => ( <FormItem><FormLabel className="text-xs">Postal/Zip</FormLabel><FormControl><Input {...field} /></FormControl></FormItem> )} />
                                                <FormField control={form.control} name="country" render={({ field }) => ( <FormItem><FormLabel className="text-xs">Country</FormLabel><FormControl><Input {...field} /></FormControl></FormItem> )} />
                                            </div>
                                        </div>
                                        <div className="space-y-2 p-4 border rounded-md bg-muted/20">
                                            <Label className="text-muted-foreground">Home Address</Label>
                                            <FormField control={form.control} name="homeAddress.street" render={({ field }) => ( <FormItem><FormLabel className="text-xs">Street</FormLabel><FormControl><Input placeholder="456 Home Ave" {...field} /></FormControl></FormItem> )} />
                                            <div className="grid grid-cols-2 gap-2">
                                                <FormField control={form.control} name="homeAddress.city" render={({ field }) => ( <FormItem><FormLabel className="text-xs">City</FormLabel><FormControl><Input {...field} /></FormControl></FormItem> )} />
                                                <FormField control={form.control} name="homeAddress.provinceState" render={({ field }) => ( <FormItem><FormLabel className="text-xs">Prov/State</FormLabel><FormControl><Input {...field} /></FormControl></FormItem> )} />
                                            </div>
                                            <div className="grid grid-cols-2 gap-2">
                                                <FormField control={form.control} name="homeAddress.postalCode" render={({ field }) => ( <FormItem><FormLabel className="text-xs">Postal/Zip</FormLabel><FormControl><Input {...field} /></FormControl></FormItem> )} />
                                                <FormField control={form.control} name="homeAddress.country" render={({ field }) => ( <FormItem><FormLabel className="text-xs">Country</FormLabel><FormControl><Input {...field} /></FormControl></FormItem> )} />
                                            </div>
                                        </div>
                                    </div>
                                </section>
      ...
      <AlertDialog open={!!industryToDelete} onOpenChange={setIndustryToDelete}>
        <AlertDialogContent>
            <AlertDialogHeader>
                ...
                <AlertDialogDescription>This will permanently delete the custom industry "{industryToDelete?.description}".</AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
