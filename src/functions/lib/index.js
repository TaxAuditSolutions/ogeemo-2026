...
exports.updateUserAuth = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (error.code === 'permission-denied' || error.code === 'app/service-account-not-found') {
            throw new functions.https.HttpsError('permission-denied', "The backend service does not have permission to update user accounts. Please grant the 'Firebase Authentication Admin' role to the service account in the Google Cloud IAM console.");
        }
